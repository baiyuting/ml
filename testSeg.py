...
def seg_word(sentence):
    ...
    # 读取停用词文件
    stopwords = set()
    fr = codecs.open('D:/20181101_审核内容情感分析/练习用资料/停用词词典.txt', 'r', 'utf-8')
    for word in fr:
...
def classify_words(word_dict):
    """词语分类,找出情感词、否定词、程度副词"""
    # 读取情感字典文件
    sen_file = open('D:/20181101_审核内容情感分析/练习用资料/情感词典.txt', 'r+', encoding='utf-8')
    # 获取字典文件内容
    ...
    # 读取否定词文件
    not_word_file = open('D:/20181101_审核内容情感分析/练习用资料/否定词.txt', 'r+', encoding='utf-8')
    # 由于否定词只有词，没有分值，使用list即可
    not_word_list = not_word_file.readlines()

    # 读取程度副词文件
    degree_file = open('D:/20181101_审核内容情感分析/练习用资料/程度副词.txt', 'r+', encoding='utf-8')
    degree_list = degree_file.readlines()
    degree_dic = defaultdict()
    # 程度副词与情感词处理方式一样，转为程度副词字典对象，key为程度副词，value为对应的程度值 设置为2
