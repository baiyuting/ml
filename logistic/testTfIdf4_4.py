...
# 读取数据
with open('D:/20181101_审核内容情感分析/export_feedback_audit_2018-11-06/data4.json', 'r', encoding='utf-8') as f:
    data3 = json.load(f)
    f.close()


with open('D:/20181101_审核内容情感分析/export_feedback_audit_2018-11-06/data6.json', 'r', encoding='utf-8') as f:
    data6 = json.load(f)
    f.close()
...
for index in range(count):
    ...
    # log_reg = EasyEnsembleClassifier(base_estimator=DecisionTreeClassifier(max_leaf_nodes=5))
    # log_reg = EasyEnsembleClassifier(base_estimator=BernoulliNB())
    # ada = AdaBoostClassifier(base_estimator=MultinomialNB())
    # log_reg = EasyEnsembleClassifier(base_estimator=ada)
    log_reg.fit(tf_idf.toarray(), target_train)
